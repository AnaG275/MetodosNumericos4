...
#   de la forma  Ax = b  usando el metodo de Eliminacion
#   Gaussiana con Pivoteo Parcial.
#
#   El metodo consta de dos grandes etapas:
#     1. Eliminacion hacia adelante: convierte la matriz
#        aumentada [A|b] en una matriz triangular superior
#        usando operaciones elementales de fila.
#     2. Sustitucion hacia atras: una vez triangular, se
#        despeja cada variable comenzando desde la ultima
#        ecuacion hacia arriba.
#
#   El Pivoteo Parcial reordena las filas en cada iteracion
#   para colocar el mayor valor absoluto como pivote, lo que
#   mejora la estabilidad numerica y evita divisiones por
...
def eliminacion_gaussiana(A, b):
    ...
    # ── Inicializacion ─────────────────────────────────────
    # n = numero de ecuaciones (e incognitas del sistema)
    n = len(b)

    # Construimos la matriz aumentada [A|b] como array flotante.
    # Cada fila i contiene los n coeficientes de A mas b[i].
    # Usamos float() para evitar errores de division entera.
    # Ejemplo para sistema 3x3:
    #   M = [[ 2,  1, -1,   8],
    #        [-3, -1,  2, -11],
    #        [-2,  1,  2,  -3]]
    M = np.array(
    ...
    # Iniciamos cronometro DESPUES de construir la matriz,
    # midiendo solo el tiempo del algoritmo en si.
    tiempo_inicio = time.perf_counter()

    # ══════════════════════════════════════════════════════
    #  ETAPA 1: ELIMINACION HACIA ADELANTE
    # ══════════════════════════════════════════════════════
    # Recorremos cada columna col = 0, 1, ..., n-1.
    # En cada iteracion, M[col][col] es el "pivote".
    # Eliminamos todos los elementos DEBAJO del pivote
    # haciendolos cero mediante operaciones de fila.
    # Al terminar, M tiene forma triangular superior.

    for col in range(n):

        # ── Pivoteo Parcial ─────────────────────────────
        # Buscamos la fila con el mayor |valor| en la columna
        # actual, entre la fila col y las que estan debajo.
        #
        # Objetivo:
        #   a) Evitar division por cero si el pivote es 0.
        #   b) Reducir errores numericos por redondeo.
        #
        # En codigo: comparamos abs(M[fila][col]) para cada
        # fila desde col hasta n-1, guardando el indice de
        # la fila mayor en la variable max_fila.
        ...
        # Si la fila mayor no es la actual, intercambiamos.
        # numpy permite intercambiar filas completas en una
        # sola instruccion: M[[i, j]] = M[[j, i]]
        if max_fila != col:
            M[[col, max_fila]] = M[[max_fila, col]]
            print(f"\n [PIVOTEO] Intercambiamos F{col+1} con F{max_fila+1}")
            imprimir_matriz(M, n)

        # ── Verificacion de pivote cero ─────────────────
        # Si M[col][col] es practicamente cero tras el pivoteo,
        # toda la columna inferior es cero. Hay dos casos:
        #
        #   INCOMPATIBLE: algun termino independiente != 0
        #                 Produce la contradiccion 0 = k.
        #   INDETERMINADO: todos los terminos son cero
        #                  Produce la identidad 0 = 0,
        #                  lo que implica infinitas soluciones.
        #
        # Usamos 1e-12 como umbral para errores de punto flotante.
        ...
        # ── Eliminacion debajo del pivote ───────────────
        # Para cada fila debajo del pivote calculamos m y
        # restamos un multiplo de la fila pivote para que
        # el elemento M[fila][col] se vuelva exactamente 0.
        #
        # Formula del multiplicador:
        #   m_ij = a_ij / a_ii
        #   donde a_ii = pivote, a_ij = elemento a eliminar
        #
        # Operacion elemental de fila:
        #   F_i <- F_i - m_ij * F_j
        #
        # En numpy: M[fila] = M[fila] - m * M[col]
        # Opera sobre toda la fila (vectorizado), incluyendo
        # la ultima columna del termino independiente.
    ...
    # ══════════════════════════════════════════════════════
    # M es ahora triangular superior. Despejamos cada variable
    # comenzando desde la ultima ecuacion (fila n-1) hacia arriba.
    #
    # Para la fila i la ecuacion es:
    #   M[i][i]*x[i] + M[i][i+1]*x[i+1] + ... = M[i][n]
    #
    # Como x[i+1]...x[n-1] ya estan calculados, sumamos sus
    # contribuciones y despejamos x[i]:
    #   x[i] = (M[i][n] - suma) / M[i][i]
    #
    # range(n-1, -1, -1) genera indices n-1, n-2, ..., 1, 0
    # es decir, recorremos las filas de abajo hacia arriba.
...
def verificar_solucion(A, b, x):
    ...
    - El error relativo evita dividir por cero usando max(abs(b[i]), 1e-12).
    - Multiplicamos por 100 para expresarlo en porcentaje.
    """
    print("\n" + "=" * 60)
    print("   VERIFICACION Y PORCENTAJE DE ERROR")
    print("=" * 60)
    print(f"   {'Ecuacion':<12} {'Calculado':>12} {'Esperado':>12} {'Error abs':>12} {'% Error':>12}  Estado")
    print("   " + "-" * 68)
    ...
    for i in range(len(b)):
        ...
        print(f"   Ec. {i+1:<8} {calculado:>12.6f} {b[i]:>12.6f} {error_abs:>12.2e} {error_pct:>11.6f}%  [{ok}]")
